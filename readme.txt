...
·如何使代码易阅读：
  ...
  -- 编写高类聚低耦合的模块：
     -- 依赖性：
        ...
        -- 当模块之间需要双向依赖时需增加中间模块，模块之间通过中间模块通讯(另外通过订阅发布或回调函数解耦依赖也是常用方式，不过方式的本质也可以看成一个中间模块)
        -- 抽象之间允许双向依赖（抽象代表了关系稳定，代码变动可能性极小）
        例如：存在这个3个页面，一个是实时播放页面1，一个录像播放页面2，他们两个都依赖了一个播放器iframe页面3。播放器加载过程要展示loading层，而实时播放页面的loading和录像页面的不同。单向依赖是指播放时，页面1和页面2都单向依赖页面3，页面1和页面2都可以调用页面3的播放方法，但是页面3不允许直接调用页面1或页面2的loading方法，而必须要通过第三方模块调用，例如window.postMessage去请求loading，window监听loading消息来调用loading方法,这里的window就是第三方模块。
  ...
  -- 减少hack、补丁、奇技淫巧：
     ...
     -- 对hack、补丁、奇技淫巧进行详细说明
  -- 优化if else的使用:
     -- 基本原则：尽可能地维持正常流程代码在最外层。意思是说，可以写 if-else 语句时一定要尽量保持主干代码是正常流程，避免嵌套过深。
     -- 实现的手段有：
        -- 减少嵌套
        -- 移除临时变量
        -- 条件取反判断
